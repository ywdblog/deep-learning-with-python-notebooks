# 1：概述
''' 
神经网络中，更新模型的权重这一步是最难的，对于模型的某个权重系数，你怎么知道这个系数应该增大还是减小，以及变化多少呢？
有个比较好的方法就是梯度下降法，它的思想是：如果你想要减小某个函数的值，那么你就应该沿着这个函数的梯度的反方向走，这样你就会朝着函数值减小的方向走，直到函数值减小到最小值为止。
...
''' 
f(x)= y 和 f(x+epsilon_x) = y + a * epsilon_x
...
# 5：mini-batch SGD vs true SGD  vs batch GD

'''
1）：小批量随机梯度下降SGD，是一种优化算法，它可以在训练神经网络时，快速找到最优解。它的基本思想是，将训练数据分成若干个小批量，然后对每个小批量进行梯度下降，每次迭代时，都随机选择一个小批量，然后计算梯度，最后将梯度应用于网络的权重，从而使网络的损失值减小。这种方法的优点是，可以利用GPU并行计算，从而大大加快训练速度。缺点是，由于每次迭代时，只使用了一个小批量的数据，所以梯度的方向并不总是准确的，因此可能会导致损失值震荡，甚至无法收敛。
2）：真SGD：是小批量随机梯度下降的一个变体，每次迭代时，都随机选择一个小批量，然后计算梯度，最后将梯度应用于网络的权重，从而使网络的损失值减小。
3）：批量梯度下降：每次迭代时，都使用所有的训练数据，然后计算梯度，最后将梯度应用于网络的权重，从而使网络的损失值减小。
'''
